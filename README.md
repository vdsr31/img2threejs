...
- **CS2 weapon review gates.** The supported path is knife-only: weapon-v1.4, manifest schema 1, `cs2-knife-v1` and `cs2-knife-review-v1`. The review records family identity, painted-region and projection coverage, per-region confidence, approximation notes and versioned knife-scene metadata; component-coverage and map-stripped blockout gates prevent a convincing texture from standing in for real structure. See [`docs/cs2/review-gates.md`](docs/cs2/review-gates.md).
